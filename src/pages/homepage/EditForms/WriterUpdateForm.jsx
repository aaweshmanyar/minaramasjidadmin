...
export default function EditWriterForm() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`https://api.minaramasjid.com/api/writers/${id}`);
        ...
        setFormData({
          ...
          joinedDate: data?.joinedDate
            ? new Date(data.joinedDate).toISOString().slice(0, 10)
            : "",
        ...
        setPreview(
          data?.imageUrl ||
            `https://api.minaramasjid.com/api/writers/image/${id}`
        );
      } catch (err) {
  ...
  const clearImage = () => {
    setProfilePic(null);
    // keep preview to show old image unless user explicitly removed it:
    setPreview(null);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        let msg = "Failed to update writer";
        try {
          const data = await res.json();
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500">Update profile details and bio.</p>
          </div>
          <button
            type="button"
        ...
        {loading ? (
          ...
          <form
            ...
            className="bg-white shadow-sm border rounded-2xl overflow-hidden"
          >
            {/* Top grid */}
            ...
            <div className="px-6 pb-6 space-y-6">
              <div>
                ...
                <ReactQuill
                  ...
                  onChange={(v) => handleDescriptionChange("englishDescription", v)}
                  modules={modules}
                  formats={formats}
              ...
              <div>
                ...
                <ReactQuill
                  ...
                  onChange={(v) => handleDescriptionChange("urduDescription", v)}
                  modules={modules}
                  formats={formats}
                  className="bg-white border rounded-xl min-h-[220px]"
            ...
            <div className="px-6 py-4 bg-gray-50 flex items-center justify-end gap-3 border-t">
              ...
              >
                {submitting ? "Saving..." : "Update Writer"}
              </button>
            </div>
          </form>
        )}
